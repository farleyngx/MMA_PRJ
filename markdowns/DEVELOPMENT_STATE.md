...
- **Sửa lỗi Native Module Null (`AsyncStorage`)**: Đã đồng bộ hóa `@react-native-async-storage/async-storage` về phiên bản `2.2.0` tương thích chính thức với Expo SDK 54 bằng lệnh `npx expo install @react-native-async-storage/async-storage`, giải quyết lỗi không tìm thấy native module khi truy cập dữ liệu đã lưu.
